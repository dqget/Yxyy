	...
	xhr.onreadystatechange = function(){
		if(xhr.readyState == 4){
			if(xhr.status == 200){
				...
				if(xhr.responseText=="用户名已经存在"){
					document.getElementById("userName").value = "";
				}
				/*
				var txt = xhr.responseText;
 				console.log("=================================="+txt);
				if(txt.indexOf("用户名已经存在")!=-1){
					document.getElementById("userName").innerHTML = "";
				} */
			}
		}
	}
	// 3.打开连接        加时间戳目的：为了消除缓存   因为用这种get方式提交请求所以不会改变地址栏的url所以会导致有hhtp缓存
...
/*	//邮箱验证
	...
	if(email==null||email==''){
		alert("邮箱不能为空!");
		return false;
	}else if(!regemail.test(email)){ //正则验证不通过，格式不对
		alert("邮箱格式不符合!");
		return false;
	}
*/
