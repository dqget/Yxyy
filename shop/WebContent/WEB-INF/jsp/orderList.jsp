				...
				<tbody>
					<s:iterator var="order" value="pageBean.list">
						<tr>
							...
							&nbsp;&nbsp;&nbsp;&nbsp;<font color="red">
								<s:if test="#order.state == 1">
									<a href="${pageContext.request.contextPath }/order_findByOid.action?oid=<s:property value="#order.oid" />"><font color="blue">未付款(去付款)</font></a>
									&nbsp;&nbsp;&nbsp;&nbsp;
									<a href="${pageContext.request.contextPath }/order_deleteOrder.action?oid=<s:property value="#order.oid" />">取消订单</a>
								</s:if>
								<s:if test="#order.state == 2">
									已付款&nbsp;&nbsp;&nbsp;&nbsp;
									<a href="#"><font color="blue">等待发货</font></a>
								</s:if>
								<s:if test="#order.state == 3">
									已发货&nbsp;&nbsp;&nbsp;&nbsp;
									<a href="${pageContext.request.contextPath }/order_takeOverOrder.action?oid=<s:property value="#order.oid" />"><font color="blue">确认收货</font></a>
								</s:if>
						...
						<s:iterator var="orderItem" value="#order.orderItems">
							<tr>
								<td width="60">
									<a href="${pageContext.request.contextPath }/product_findByPid.action?pid=<s:property value="#orderItem.product.pid" />"><img src="${ pageContext.request.contextPath }/<s:property value="#orderItem.product.image"/>" /></a>
								</td>
								...
								<td width="140">
									<span class="subtotal">￥<s:property value="#orderItem.subtotal" />
									</span>
								</td>
								<td>
									<s:if test="#order.state == 4">
										<s:if test="#orderItem.evastate==0||#orderItem.evastate==null">
											<a href="${pageContext.request.contextPath }/evaluate_enterEva.action?pid=<s:property value="#orderItem.product.pid" />&itemid=<s:property value="#orderItem.itemid" />"><font color="blue">评论商品</font></a>
										</s:if>
										<s:else>
											<a href="${pageContext.request.contextPath }/product_findByPid.action?pid=<s:property value="#orderItem.product.pid" />"><font color="red">已评论</font></a>
										</s:else>
									</s:if>
									<s:else><font color="red">交易未完成不能评论</font></s:else>
								</td>
							</tr>
						</s:iterator>
					</s:iterator>
