...
<h1>标题</h1><br><br><br>
<section>
<a href="FindOrderByCname.jsp">客户预定信息</a>
<a href="FindOrderByPhone.jsp">电话预定信息</a>
<a href="FindOrderByFloor.jsp">楼层预定信息</a>
<a href="FindOrderByRid.jsp">房间预订信息</a>
<form action="OrderManage.jsp" method="post">
<select name="rtype">
<option value="0">大床房</option>
<option value="1">标间</option>
<option value="2">豪华套房</option>
</select>
<input name="start" type="date" value="${param.start }"/>
<input name="end" type="date" value="${param.end }"/>
<input type="submit" value="提交"/>
</form>
	<c:set var="rtype" value="${param.rtype }" />
	<c:set var="start" value="${param.start }" />
	<c:set var="end" value="${param.end }" />
	<jsp:useBean id="OrderRoomBean" class="bean.OrderRoomBean" />
	<c:set var="orderrooms" value="${OrderRoomBean.getOrderRooms(rtype,start,end) }"></c:set>
	<table>
		<caption>可预订房间列表</caption>
		<tr>
			<th>楼层号</th>
			<th>房间号</th>
			<th>标准</th>
		...
		<c:forEach var="orderroom" items="${orderrooms }" varStatus="s">
			<tr>
				<td>${orderroom.floor }</td>
				<td>${orderroom.rid }</td>
				<td>${orderroom.standard }</td>
				<td>${orderroom.charge }</td>
				<td>${orderroom.position }</td>
				<td><a href="OrderRoom.jsp?rid=${orderroom.rid }">预定</a></td>
			</tr>
		</c:forEach>
	</table>
</section>
