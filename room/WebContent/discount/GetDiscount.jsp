<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8" import="java.util.*" import="jdbc.*"%>
...
<h1>标题</h1><br><br><br>
	<table border="1">
		...
		<%
			List<discountBean> list = (List) request.getAttribute("list");
			if(list == null){
				return;
			}else{
		...
		%>
		<tr>
			<td><%=list.get(i).getMemberType()%></td>
			<td><%=list.get(i).getDiscount()%></td>
			<td><a href="updatediscount?id=<%=list.get(i).getMemberType()%>">修改</a>
			<td><a href="deletediscount?id=<%=list.get(i).getMemberType()%>">删除</a>
		</tr>
		<%
				}
			}
